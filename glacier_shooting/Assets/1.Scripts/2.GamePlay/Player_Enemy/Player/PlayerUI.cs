...
public class PlayerUI : MonoBehaviour
{
    ...
    void Awake()
    {
        if (status == null) status = FindFirstObjectByType<PlayerStatus>();
    ...
    private void Update()
    {
        ...
        {
            string hpCurrent = SafeInt(() => status.PlayerHealth);
    ...
    public void RefreshStats(float MoveSpeed, int MaxHealth, float AttackPower, float FireRate, int Luck)
    {
        if (Text_Character != null)
            Text_Character.text = $"{Label_Character} : {(status?.player != null ? status.player.playerName : "-")}";
